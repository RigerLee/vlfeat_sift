                 ...
                 double sigma)
{
  ...
  if (self->gaussFilterSigma != sigma) {
    vl_uindex j ;
    vl_sift_pix acc = 0 ;
    if (self->gaussFilter) free (self->gaussFilter) ;
    self->gaussFilterWidth = VL_MAX(ceil(4.0 * sigma), 1) ;
    self->gaussFilterSigma = sigma ;
    self->gaussFilter = malloc (sizeof(vl_sift_pix) * (2 * self->gaussFilterWidth + 1)) ;
...
vl_sift_delete (VlSiftFilt* f)
{
  if (f) {
    ...
    if (f->gaussFilter) free (f->gaussFilter) ;
    free (f) ;
  }
}
...
// original: void VL_XCAT3(_vl_imconvcol_v, SFX, _sse2), see vl/imopv_sse2.c
void
vl_imconvcol_vf_sse2 (vl_sift_pix* dst, vl_size dst_stride,
                      vl_sift_pix const* src,
...
// #define VSTZ  VL_XCAT(_mm_setzero_p, VSFX) and VSFX = s
  ...
  while (x < (signed)src_width) {
        ...
        VALIGNED(src + x) & use_simd)
    {
      ...
      for (y = 0 ; y < (signed)src_height ; y += step)  {
        union {VTYPE v ; vl_sift_pix x [VSIZE] ; } acc ;
        VTYPE v, c ;
        vl_sift_pix const *srci ;
        acc.v = VSTZ () ;
        v = VSTZ() ;
...
VL_EXPORT void
vl_imconvcol_vf (vl_sift_pix* dst, vl_size dst_stride,
                 vl_sift_pix const* src,
                 vl_size src_width, vl_size src_height, vl_size src_stride,
                 vl_sift_pix const* filt, vl_index filt_begin, vl_index filt_end,
                 int step, unsigned int flags)
{
  ...
  vl_imconvcol_vf_sse2 (dst,dst_stride,
